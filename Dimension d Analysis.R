...
for(rep in 1:repetitions){
  ...
  for (t in 1:Tb) {
    ...
    for (i in 1:n) {
      ...
      for (j in 1:number_basis) {
        ...
        #for(k in 1:d)
        #{
        # aux_1=(sqrt(2)/2*pi)*sin(j*X_s[aux-nts[t]+i,k])*(aux_1)*(1/2)
        #}
  ...
  h_int=seq(0.9999,1.0001,0.0001)
  l_h_int=length(h_int)
  tau_int=c(0.1899,0.19,0.1901)
...
#FSBS
ave_scale_hd=sum_scale_hd/repetitions
ave_hd=sum_hd/repetitions
ave_kd=sum_kd/repetitions
...
for(rep in 1:repetitions){
  ...
  h_int=seq(0.9999,1.0001,0.0001)
  l_h_int=length(h_int)
  tau_int=c(0.004599,0.0046,0.046001)
  l_tau_int=length(tau_int)
...
for(rep in 1:repetitions){
  ...
  h_int=seq(0.9999,1.0001,0.0001)
  l_h_int=length(h_int)
  tau_int=c(0.0004599,0.00046,0.0046001)
  l_tau_int=length(tau_int)
