...
for(rep in 1:repetitions){
  ...
  #######################################################################################
  ############################## Runing with the parameters chosen ###################### 
  ...
  no_grids=n-1
  time3 <- proc.time()
  EJS_result <- SN_BS_functional(data=dat_1, d=d_PC, num_basis=10, rangeval=c(0,1), no_grids=no_grids,
  ...
  # BGHK
  time5 <- proc.time()
  BGHK_result <- BGHK_BS_functional(data=dat_1, d=d_PC, num_basis=10, rangeval=c(0,1), no_grids=no_grids,
  ...
  # HK
  time7 <- proc.time()
  HK_result <- HK_BS_functional(data=dat_1, d=d_PC, num_basis=10, rangeval=c(0,1), no_grids=no_grids,
                                start=1, end=Tb, critical_value=crit_bghk)
  time8 <- proc.time()
